...
def _save_profile_sync(profile: dict) -> None:
    ...
    try:
        ...
        with get_sync_db() as db:
            ...
            # PRD v1.2: includes all 19+ profile fields
            query = text("""
            ...
            db.execute(query, {
                ...
                "executives": json.dumps(profile.get("executives", [])),
                "industry_overview": profile.get("industry_overview"),
                "business_model": profile.get("business_model"),
                "financial_history": json.dumps(profile.get("financial_history", [])),
                "competitors": json.dumps(profile.get("competitors", [])),
                "macro_factors": json.dumps(profile.get("macro_factors", [])),
                "supply_chain": json.dumps(profile.get("supply_chain", {})),
                "overseas_business": json.dumps(profile.get("overseas_business", {})),
                "shareholders": json.dumps(profile.get("shareholders", [])),
                "consensus_metadata": json.dumps(profile.get("consensus_metadata", {})),
                # Original fields
...
def run_analysis_pipeline(self, job_id: str, corp_id: str, skip_cache: bool = False):
    ...
    # P0-FIX: 해커톤 시연용으로 Legacy mode 사용 (Multi-Agent hang 이슈)
